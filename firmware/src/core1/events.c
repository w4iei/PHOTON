...
void events_process(const uint16_t *readings, uint32_t now_us) {
    ...
    for (int i = 0; i < PHOTON_ACTIVE_SENSORS; i++) {
        ...
        if (!e->note_on[i]) {
            ...
            // (A jump past both thresholds in one sweep arms and fires in
            // the same iteration — dt 0, the fastest measurable strike.)
            if (!e->strike_pending[i] && press >= vel_start_thr) {
