...
class FrameDispatcher:
    ...
    def send_minmax_dump(self, target_id: int, seq: int) -> int:
        ...
        while start < self.active_sensors:
            ...
            self.bus.send_frame(FRAME_TYPE_MINMAX_RESP, target_id, resp, seq & 0xFFFF, ack_timeout_us=0)
            seq = (seq + 1) & 0xFFFF
            start += count
        return seq

    def handle_frames(self) -> None:
        for frame_type, target_id, source_id, payload, seq in self.bus.read_frames():
            if target_id not in (self.device_id, 0):
                continue
            self.counters["rx"] += 1
            if frame_type == FRAME_TYPE_STATS_REQ:
                now = time.monotonic()
                ...
                self.bus.send_frame(FRAME_TYPE_STATS_RESP, source_id, resp_payload, seq, ack_timeout_us=0)
                ...
                cmd = payload[0]
                if cmd == CAL_CMD_SAVE:
                    self.send_minmax_dump(source_id, (seq + 1) & 0xFFFF)
                ...
                self.bus.send_frame(FRAME_TYPE_MINMAX_RESP, source_id, resp, seq, ack_timeout_us=0)
                self.counters["resp"] += 1
                ...
                self.bus.send_frame(FRAME_TYPE_TRACE_RESP, source_id, resp, seq, ack_timeout_us=0)
                self.counters["resp"] += 1
